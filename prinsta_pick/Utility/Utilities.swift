...
class Utilities: NSObject {
    ...
    class  func setRightViewIcon(icon: UIImage,field:UITextField) {
        ...
        let imageview = UIImageView(frame: CGRect(x: 6, y: 0, width: (height * 0.50), height: (height * 0.50)))
    ...
    class  func setRightSideSliderView(field:UITextField,value : Bool) {
        let height = field.frame.height
        ...
        let view = UIView(frame: CGRect(x: 0, y:0, width: 50, height: h))
        let slider = UISwitch(frame: CGRect(x: 0, y: 3, width: 20, height: (height * 0.10)))
        view.backgroundColor = .clear
        slider.tintColor = .lightGray
        slider.onTintColor =  CustomColor.appThemeColor
        ...
        slider.isOn = value
        if value {
            slider.tintColor = CustomColor.appThemeColor
        } else {
            slider.tintColor = .lightGray
        }
        slider.layer.cornerRadius = slider.frame.height/2 + 3
        view.addSubview(slider)
        field.rightViewMode = .always
        field.rightView = view
    }
    
    class  func setleftViewIcon(icon: UIImage,field:UITextField) {
        ...
        let view = UIView(frame: CGRect(x: 5, y:0, width: 30, height: h))
        
        let imageview = UIImageView(frame: CGRect(x: 6, y:Int(Int(h/2 ) - Int(height/2)), width: width, height: height))
...
//        }
    ...
    class  func setleftViewIconOnUpperSide(icon: UIImage,field:UITextField) {
   ...
   class func getCurrentTime() -> String {
    ...
    class func getCurrentMilliSecond()->Int64{
    ...
    class func timestampToDate(timestamp : Double) -> String{
        ...
        let str = convertToString(date: date, formatOut: AppHelper.getStringForKey(ServiceKeys.dateFormat))
        return str
    }
   ...
   class func getTimeHoursDataBetweenTwoTimes() -> String {
        let loginTime = UserDefaults.standard.object(forKey: ServiceKeys.clock_in) as? Date ?? Date()
   ...
   class func timeRemainingString(date:NSDate) -> String {
    ...
    class func isValidName(testStr:String) -> Bool {
        do {
            ...
            if regex.firstMatch(in: testStr, options: [], range: NSMakeRange(0, testStr.characters.count)) != nil {
                return false
